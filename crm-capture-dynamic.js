(function () {
  ...
  function isPlaceholderVirtualAgentIdentity(name, id) {
    const normalizedName = String(name || "")
      .trim()
      .toLowerCase()
      .replace(/\s+/g, " ");
    const normalizedId = String(id || "").trim().toLowerCase();
    return normalizedName === "pulse ai" || normalizedId === "pulse_welcome";
  }
  ...
  function maybeApplyVirtualAgentFromEvent(evt, eventConversationId) {
    let name = extractVirtualAgentDisplayName(evt);
    let id = extractVirtualAgentId(evt);
    let profileKey = extractVirtualAgentProfileKey(evt);
    if (!name && !id && !profileKey) return;
    if (isPlaceholderVirtualAgentIdentity(name, id)) {
      name = null;
      id = null;
      profileKey = null;
    }
    ...
    if (!name && !id && profileKey === null) return;
