(function () {
  ...
  async function initializeConversation(userDetails = {}) {
        ...
        try {
          ...
          if (restoreRes.ok) {
            ...
            if (data.conversation) {
              ...
              // If found, just render history. Done.
    ...
    // B. CREATE NEW CONVERSATION (If restore failed)
    try {
      ...
      // C. FETCH THREAD AGAIN (After Creation)
      // This ensures we pull any system/welcome message generated by the backend during creation
      if (!settings.testMode) {
          try {
              ...
              if (threadRes.ok) {
                  ...
                  if (threadData.messages && threadData.messages.length > 0) {
                      // Backend generated a message, show it
                      threadData.messages.forEach(msg => appendMessageToUI(msg.text, msg.sender));
                  } else {
                      // D. LOCAL FALLBACK
                      // Backend sent nothing. Show local welcome message.
          ...
          // Test Mode: Just show local welcome message
          const welcomeText = settings.appearance.header?.welcomeMessage || settings.appearance.welcomeMessage;
          if (welcomeText) appendMessageToUI(welcomeText, 'agent');
      }
  ...
  function connectSocket() {
    ...
    // Connect using explicit path options
    const options = {
