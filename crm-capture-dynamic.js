      ...
      if (currentView === "form") {
        ...
        const fieldsHtml = settings.preChatForm.fields
          .map((f) => {
            ...
            if (f.type === "textarea") {
              inputHtml = `<textarea class="chat-widget-form-input" name="${f.id}" placeholder="${f.label}"></textarea>`;
            } else {
              const inputType = f.type === "phone" ? "tel" : f.type;
              inputHtml = `<input class="chat-widget-form-input" type="${inputType}" name="${f.id}" placeholder="${f.label}">`;
            }
