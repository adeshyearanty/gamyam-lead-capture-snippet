(function () {
    ...
    function normalizeSocketMessagePayload(message) {
        ...
        normalized.text =
            ...
            payload.text ??
            payload.body ??
            extractAssistantTextFromPayload(source) ??
            extractAssistantTextFromPayload(payload) ??
            message.text;
    ...
    function formatFormSubmissionText(formData, fields) {
        if (!formData || typeof formData !== "object") return "";
        const labelByKey = {};
        if (Array.isArray(fields)) {
            fields.forEach((field) => {
                if (!field || !field.key) return;
                labelByKey[field.key] = field.prompt || field.key;
            });
        }
    ...
    function tableBlockToMarkdown(tableData) {
        const allColumns = Array.isArray(tableData?.columns) ? tableData.columns : [];
        const visibleColumns = allColumns.filter((column) => !column?.hidden);
        const rows = Array.isArray(tableData?.rows) ? tableData.rows : [];
        if (visibleColumns.length === 0 || rows.length === 0) return "";

        const header = `| ${visibleColumns
            .map((column) => String(column?.label || column?.key || ""))
            .join(" | ")} |`;
    ...
    function contentBlocksToText(contentBlocks) {
        if (!Array.isArray(contentBlocks) || contentBlocks.length === 0) return "";
        const contentParts = [];
        contentBlocks.forEach((block) => {
            if (block?.format === "markdown" && typeof block?.text === "string") {
            ...
            if (
                block?.format === "json" &&
                block?.visual_type === "table" &&
                block?.data
            ) {
                const tableMarkdown = tableBlockToMarkdown(block.data);
                if (tableMarkdown) contentParts.push(tableMarkdown);
            }
        });
        return contentParts.join("\n\n");
    }

    function extractAssistantTextFromPayload(source) {
        if (!source || typeof source !== "object") return null;
        const fromBlocks = contentBlocksToText(
            source.content_blocks || source.contentBlocks,
        );
    ...
    function renderInlineMarkdown(escapedText) {
        ...
        out = out.replace(/`([^`\n]+)`/g, '<code class="chat-md-code-inline">$1</code>');
        out = out.replace(/\*\*([^*\n]+)\*\*/g, "<strong>$1</strong>");
        out = out.replace(/\*([^*\n]+)\*/g, "<em>$1</em>");
        out = out.replace(
            /\[([^\]]+)\]\(([^)]+)\)/g,
            (_, label, url) =>
                `<a href="${escapeHtmlWidget(sanitizeMarkdownLink(url))}" target="_blank" rel="noopener noreferrer">${label}</a>`,
        );
        return out;
    }
    ...
    function renderMarkdownTableLines(lines) {
        ...
        const parseRow = (row) =>
            String(row || "")
                ...
                .replace(/\|$/, "")
                .split("|")
                .map((cell) => renderInlineMarkdown(escapeHtmlWidget(cell.trim())));
        ...
        const bodyHtml = bodyRows
            ...
            .join("");
        return `<div class="chat-md-table-wrap"><table class="chat-md-table"><thead><tr>${headHtml}</tr></thead><tbody>${bodyHtml}</tbody></table></div>`;
    }
    ...
    function renderChatMarkdownToHtml(markdown) {
        ...
        while (i < lines.length) {
            ...
            if (line.trim().startsWith("```")) {
                ...
                const code = escapeHtmlWidget(codeLines.join("\n"));
                const langClass = lang ? ` class="language-${escapeHtmlWidget(lang)}"` : "";
                parts.push(`<pre class="chat-md-pre"><code${langClass}>${code}</code></pre>`);
                continue;
            }
            ...
            if (isMarkdownTableRow(line) && i + 1 < lines.length && isMarkdownTableSeparator(lines[i + 1])) {
                ...
                parts.push(renderMarkdownTableLines(tableLines));
                continue;
            }
            ...
            if (headingMatch) {
                ...
                parts.push(
                    `<h${level} class="chat-md-h${level}">${renderInlineMarkdown(
                        escapeHtmlWidget(headingMatch[2].trim()),
                    )}</h${level}>`,
                );
                i += 1;
                continue;
            ...
            if (/^\s*[-*]\s+/.test(line)) {
                ...
                while (i < lines.length && /^\s*[-*]\s+/.test(lines[i])) {
                    ...
                    items.push(
                        `<li><span class="chat-md-bullet" aria-hidden="true">•</span><span>${renderInlineMarkdown(
                            escapeHtmlWidget(itemText),
                        )}</span></li>`,
                    );
                    i += 1;
                }
            ...
            if (/^\s*\d+\.\s+/.test(line)) {
                ...
                while (i < lines.length && /^\s*\d+\.\s+/.test(lines[i])) {
                    ...
                    items.push(
                        `<li>${renderInlineMarkdown(escapeHtmlWidget(itemText))}</li>`,
                    );
                    i += 1;
                }
                parts.push(`<ol class="chat-md-ol">${items.join("")}</ol>`);
                continue;
            }
            ...
            if (/^\s*>\s?/.test(line)) {
                ...
                parts.push(
                    `<blockquote class="chat-md-blockquote">${renderInlineMarkdown(
                        escapeHtmlWidget(quoteLines.join(" ")),
                    )}</blockquote>`,
                );
                continue;
            }
            ...
            parts.push(
                `<p>${renderInlineMarkdown(escapeHtmlWidget(paragraphLines.join(" ")))}</p>`,
            );
        }
    ...
    function appendMessageToUI(
        ...
        const popupFormConfigEarly =
            !options.suppressPopupForm &&
            type === "agent" &&
            extractPopupFormConfig(normalizedFlow);
        if (popupFormConfigEarly) {
            const popupMessageData = {
                id: normalizedId,
                messageId: normalizedId,
                text: normalizedText,
                sender: type,
            ...
            messages.set(normalizedId, popupMessageData);
            if (messageId && normalizedId !== messageId) {
                messages.set(messageId, popupMessageData);
            }
            activePopupFormConfig = popupFormConfigEarly;
            popupFormValues = {};
            popupFormError = "";
    ...
    function renderWidget() {
        ...
        styleTag.textContent = `
          ...
          .chat-widget-window {
            ...
            opacity: 0;
            pointer-events: none;
            transform: ${isTop ? "translateY(-20px)" : "translateY(20px)"
            } scale(0.95);
          ...
          .chat-widget-message-content .chat-message-markdown > :first-child {
            margin-top: 0;
          }
          .chat-widget-message-content .chat-message-markdown > :last-child {
            margin-bottom: 0;
          }
          .chat-widget-message-content .chat-message-markdown p {
            margin: 0 0 0.5rem;
          }
          .chat-widget-message-content .chat-message-markdown p:last-child {
            margin-bottom: 0;
          }
          .chat-widget-message-content .chat-message-markdown h1,
          .chat-widget-message-content .chat-message-markdown .chat-md-h1 {
            margin: 1rem 0 0.75rem;
            font-size: 14px;
            font-weight: 700;
            color: #1f2937;
          }
          .chat-widget-message-content .chat-message-markdown h2,
          .chat-widget-message-content .chat-message-markdown .chat-md-h2,
          .chat-widget-message-content .chat-message-markdown h3,
          .chat-widget-message-content .chat-message-markdown .chat-md-h3,
          .chat-widget-message-content .chat-message-markdown h4,
          .chat-widget-message-content .chat-message-markdown .chat-md-h4 {
            margin: 0.75rem 0 0.5rem;
            font-size: 14px;
            font-weight: 600;
            color: #1f2937;
          }
          .chat-widget-message-content .chat-message-markdown h5,
          .chat-widget-message-content .chat-message-markdown .chat-md-h5,
          .chat-widget-message-content .chat-message-markdown h6,
          .chat-widget-message-content .chat-message-markdown .chat-md-h6 {
            margin: 0.5rem 0 0.25rem;
            font-size: 12px;
            font-weight: 500;
            color: #6b7280;
          }
          .chat-widget-message-content .chat-message-markdown .chat-md-list,
          .chat-widget-message-content .chat-message-markdown .chat-md-ol {
            margin: 0 0 0.75rem;
            padding: 0;
            list-style: none;
          }
          .chat-widget-message-content .chat-message-markdown .chat-md-list li,
          .chat-widget-message-content .chat-message-markdown .chat-md-ol li {
            display: flex;
            align-items: flex-start;
            gap: 0.5rem;
            margin-bottom: 0.25rem;
          }
          .chat-widget-message-content .chat-message-markdown .chat-md-bullet {
            color: #6b7280;
            line-height: 1.5;
          }
          .chat-widget-message-content .chat-message-markdown .chat-md-code-inline {
            border-radius: 4px;
            background: #f3f4f6;
          ...
          .chat-widget-message-content .chat-message-markdown .chat-md-pre {
            margin: 0.75rem 0;
            overflow-x: auto;
            border-radius: 4px;
            background: #f3f4f6;
          ...
          .chat-widget-message-content .chat-message-markdown .chat-md-pre code {
            font-family: ui-monospace, SFMono-Regular, Menlo, Monaco, Consolas, monospace;
            font-size: 12px;
            color: #374151;
            white-space: pre-wrap;
          }
          .chat-widget-message-content .chat-message-markdown .chat-md-blockquote {
            margin: 0.75rem 0;
          ...
          .chat-widget-message-content .chat-message-markdown .chat-md-table {
            width: 100%;
            border-collapse: collapse;
            font-size: 14px;
          }
          .chat-widget-message-content .chat-message-markdown .chat-md-table th,
          .chat-widget-message-content .chat-message-markdown .chat-md-table td {
  ...
  <stop offset="0.14" stop-color="#21C8FF"/>
      ...
      `;
        ...
        const rebuildChatThreadFromStore = () => {
        ...
        `;
          ...
          `;
                ...
                formEl.addEventListener("submit", (event) => {
                    ...
                    renderView();
                    const submittedPopupFields = activePopupFormConfig.fields;
                    const submissionText = formatFormSubmissionText(
                        formData,
                        submittedPopupFields,
                    );
                    const fieldMappings = activePopupFormConfig.fields.map((field) => ({
                        key: field.key,
                        prompt: field.prompt || field.key,
                        crmPath: field.crmPath || "",
                        inputType: field.inputType,
                    }));
                    sendMessageToApi(submissionText || "Form submitted", {
                        interactivePayload: {
                        ...
                        .then(() => {
                            ...
                            renderView();
                            appendMessageToUI(
                                submissionText || "Form submitted",
                                "user",
                                `form_submit_${Date.now()}_${Math.random().toString(36).slice(2, 8)}`,
                                new Date(),
                                "sent",
                                null,
                                false,
                ...
                if (messages.size > 0) {
                    rebuildChatThreadFromStore();
                } else if (!staticWelcomeShown) {
                    const welcomeText =
                        settings.appearance.header?.welcomeMessage ||
                        settings.appearance.welcomeMessage;
